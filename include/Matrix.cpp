...
public:
    ...
    Matrix<T> operator*(const Matrix<T>& matrix1/*, const Matrix<T>& matrix2*/): boost::numeric::ublas::matrix<T>::operator*(&matrix1/*,matrix2*/)  { }
};
...
template <class T>
Matrix<std::string> operator*(const Matrix<std::string>& matrix2, const Matrix<T>& matrix1)
{
    if (matrix1.size2() == matrix2.size1())
    {
        Matrix<std::string> result(matrix1.size1(), matrix2.size2());
